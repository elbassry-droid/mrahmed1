...
export const PdfViewerModal: React.FC<PdfViewerModalProps> = ({ pdf, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-2 sm:p-4 overflow-y-auto animate-fadeIn select-none">
      <div className="relative w-full max-w-5xl bg-[#1a2c23] text-white rounded-3xl overflow-hidden border border-emerald-800/60 shadow-2xl flex flex-col my-auto max-h-[92vh]">
        
        {/* Top Controls Toolbar */}
        <div className="p-4 bg-[#14231b] border-b border-emerald-900/40 flex items-center justify-between gap-4">
          <button
            onClick={onClose}
        ...
        {/* PDF Stage Container */}
        <div className="flex-1 bg-[#23382d] overflow-auto p-4 sm:p-8 flex items-center justify-center relative">
          
          {/* Simulated PDF Paper Page */}
          <div 
            className="bg-white text-gray-900 shadow-2xl rounded-xl p-8 sm:p-12 w-full max-w-2xl min-h-[580px] flex flex-col justify-between relative transition-all duration-300"
            style={{ transform: `scale(${zoomLevel / 100})`, transformOrigin: 'top center' }}
          >
            {/* Watermark across paper */}
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-5 rotate-[-25deg] select-none text-4xl sm:text-5xl font-black text-[#1b4332]">
              منصة القائد - مستر أحمد عبدالحميد
            </div>
            ...
            <div className="py-6 space-y-4 text-right text-xs sm:text-sm leading-relaxed text-gray-800">
            ...
            <div className="border-t border-gray-200 pt-3 flex items-center justify-between text-[10px] text-gray-500">
              <span>جميع الحقوق محفوظة لمنصة القائد © 2026</span>
              <span className="font-bold text-[#2d6a4f]">مستر أحمد عبدالحميد - 01559196263</span>
            </div>
